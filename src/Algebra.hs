...
type HasqlAlgebra hasql init up table column
    ...
    (
        ...
        -- Statement
        (
            String -> expression -> statement, -- Declaration
            String -> expression -> statement, -- Assignment
            operation -> [argument] -> statement
        ),
...
foldHasql (hasql1, init1, table1,
    ...
    where
        ...
        fStatement (Declaration s e) = declstat s (fExpression e)
        ...
        fExpression (Expr e1 op e2) = operexpr (fExpression e1) (operator1 op) (fExpression e2)
        fExpression (Conditional e1 e2 e3) = condexpr (fExpression e1) (fExpression e2) (fExpression e3)
